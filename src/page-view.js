...
export class PageView extends ViewBase {
  ...
  tagScrollItemClick(e) {
    ...
    if (
      ...
      !e.metaKey &&
      !e.shiftKey &&
      !e.ctrlKey // let the user open in a new tab
    ) {
  ...
  initPageProperties() {
    ...
    if (this.notFound) {
      ...
      if (this.tags && this.tags.length) {
        this.selectedTag =
          this.locationPath === this.pageKey ? this.tags[0].key : this.locationPath;
      } else {
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        item-list.card-icon h2:first-child,
        item-list.card-hero h2:first-child {
          margin-top: 12px;
        }
        item-list.card-icon content-item,
        item-list.card-hero content-item {
  ...
  get templateElementClasses() {
    return this.notFound ? 'notFound' : `${this.color} ${this.shape}`;
  }
    ...
    if (this.sectionFormat === 'dropdown') {
      ...
      return html`
        ${(currentSection.items || []).map(
          item => html` <content-item .item=${item} .format=${this.itemFormat}></content-item> `,
        )}
      `;
    }
  ...
  get mainTemplate() {
    ...
    return html`
      ...
      <page-header>
        ...
        <hh-doodle .shape=${this.shape} class="${this.color} ${this.shape}"></hh-doodle>
      </page-header>
      ...
      ${!this.summary
        ...
        : html`
          ...
          `}
      ${this.sections && this.sectionFormat === 'dropdown'
        ? html`
            <!-- #=== TAG DROPDOWN ===# -->
            <tag-dropdown>
              <hh-select
      ...
      <item-list>
        ...
        <doodle-wrapper>
          <css-doodle class="not-found">
            :doodle { flex: none; @grid: 12x16; @size: 800px 600px; overflow: hidden;
            background-color: var(--blue-0); } :container { animation: container 8s linear infinite
            both; } overflow: hidden; opacity: .85; @row(even) { animation: rowEven 8s ease both
            infinite @r(0, .15s); ::before { animation: before 8s ease both infinite @r(0, .15s); }
            ::after { animation: after 8s ease both infinite @r(0, .15s); } @odd { ::before {
            background-image: linear-gradient(var(--blue-5) 50%, var(--blue-4) 50%); } ::after {
            background-image: linear-gradient(var(--blue-6) 50%, var(--blue-2) 50%); } } @even {
            ::before { background-image: linear-gradient(90deg, var(--green-7) 50%, var(--green-3)
            50%); } ::after { background-image: linear-gradient(90deg, var(--green-4) 50%,
            var(--green-2) 50%); } } } @row(odd) { animation: rowOdd 8s ease both infinite @r(2s,
            2.15s); ::before { animation: before 8s ease both infinite @r(2s, 2.15s); } ::after {
            animation: after 8s ease both infinite @r(2s, 2.15s); } @odd { ::before {
            background-image: linear-gradient(var(--indigo-4) 50%, var(--indigo-3) 50%); } ::after {
            background-image: linear-gradient(var(--indigo-5) 50%, var(--indigo-7) 50%); } } @even {
            ::before { background-image: linear-gradient(90deg, var(--purple-4) 50%, var(--purple-2)
            50%); } ::after { background-image: linear-gradient(90deg, var(--purple-6) 50%,
            var(--purple-5) 50%); } } } ::before { content: ""; position: absolute; top: 0; left: 0;
            @size: 100%; } ::after { content: ""; position: absolute; top: 50%; left: 50%; @size:
            80%; margin-top: -40%; margin-left: -40%; border-radius: 100%; } @keyframes rowEven { 0%
            { transform: translate(50%, 0);} 6% { transform: translate(155%, 0);} 10% { transform:
            translate(150%, 0);} 50% { transform: translate(150%, 0);} 55% { transform:
            translate(45%, 0);} 60% { transform: translate(50%, 0);} 100% { transform:
            translate(50%, 0); } } @keyframes rowOdd { 0% { transform: translate(0%, 0);} 6% {
            transform: translate(105%, 0);} 10% { transform: translate(100%, 0);} 50% { transform:
            translate(100%, 0);} 55% { transform: translate(-5%, 0);} 60% { transform: translate(0%,
            0);} 100% { transform: translate(0%, 0); } } @keyframes before { 0% { transform:
            rotate(0deg) } 6% { transform: rotate(95deg) } 10% { transform: rotate(90deg) } 50% {
            transform: rotate(90deg) } 55% { transform: rotate(-5deg) } 60% { transform:
            rotate(0deg) } } @keyframes after { 0% { transform: rotate(0deg) } 6% { transform:
            rotate(-95deg) } 10% { transform: rotate(-90deg) } 50% { transform: rotate(-90deg) } 55%
            { transform: rotate(5deg) } 60% { transform: rotate(0deg) } } @keyframes container { 0%
            { transform: scale(3) rotate(-45deg) translate3d(50px, -50px, 0); } 100% { transform:
            scale(3) rotate(-45deg) translate3d(-50px, 50px, 0); } }
          </css-doodle>
        </doodle-wrapper>
